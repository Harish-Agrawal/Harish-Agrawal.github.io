...
function loadSkills(skills){
		...
		while(i<skills.length){
			...
			for(j=i;j<i+6&&j<skills.length;j++){
				var skill = '<div class="col m2"><svg viewBox="0 0 128 128"><img src="'+skills[j].icon+'"></img></svg>'+skills[j].name+'</div>';
				row+=skill;
    ...
    for (i = 0; i < industryExperience.length; i++) {
        ...
        if (exp.projects && exp.projects.length > 0) {
            for (j = 0; j < exp.projects.length; j++) {
                var project = exp.projects[j]; // Using singular 'project'
                expInnerHTML += `
                    <div class="row" style="margin-left: 20px; margin-top: 15px;">
                        <div class="title" style="font-size: 1.1em;">${project.title}</div>
                        <ul style="margin-top: 10px;">`;
                
                // Fixed: Access project details from the individual project object
                for (var k = 0; k < project.details.length; k++) {
                    expInnerHTML += `<li>${project.details[k]}</li>`;
                }
                expInnerHTML += `</ul></div>`;
            }
        }
